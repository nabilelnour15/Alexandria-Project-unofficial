...
const StatusBadge = ({ status }: { status: Project['status'] }) => {
  const styles = {
    ...
    'Detailed Study Required': 'bg-orange-100 text-orange-700 border-orange-200'
  };

  const Icon = status === 'Completed' ? CheckCircle2 : status === 'Detailed Study Required' ? AlertCircle : Clock;

  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider border ${styles[status] || styles['Planning']}`}>
  ...
  return (
    ...
    >
      <div className="relative h-56 overflow-hidden">
        <PlaceholderImage text={project.imagePlaceholder} className="h-full group-hover:scale-105 transition-transform duration-700" />
      ...
      <div className="p-6 flex flex-col flex-grow">
        <div className="mb-4">
          <span className="text-blue-600 text-xs font-bold uppercase tracking-widest mb-1 block">{project.subCategory || project.category}</span>
        ...
        <div className="flex items-center justify-between pt-6 border-t border-slate-100 mt-auto">
          <div className="flex flex-col">
            ...
            <span className="text-slate-900 font-bold text-sm">{project.budget}</span>
          </div>
          <div className="flex flex-col text-right">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Timeline</span>
            <span className="text-slate-900 font-bold text-sm">{project.year}</span>
          </div>
        </div>
      </div>
    </motion.div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
          ...
          >
            ...
            <div className="md:w-1/3 relative h-64 md:h-auto bg-slate-50 border-r border-slate-100 overflow-y-auto custom-scrollbar">
              <PlaceholderImage text={project.imagePlaceholder} className="h-64 md:h-1/2 w-full" />
              <div className="p-8">
                <StatusBadge status={project.status} />
                <h2 className="text-2xl font-black text-slate-900 font-['Montserrat'] leading-tight mt-4 mb-2">{project.title}</h2>
                ...
                {project.quote && (
                  <blockquote className="mt-8 pt-8 border-t border-slate-200">
                    <p className="text-slate-600 italic text-sm leading-relaxed">"{project.quote}"</p>
                  </blockquote>
                )}
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 md:p-10 custom-scrollbar space-y-10 bg-white">
              ...
              <section>
                <h4 className="flex items-center gap-2 text-lg font-bold text-slate-900 font-['Montserrat'] mb-4">
                  <span className="w-1 h-6 bg-blue-600 rounded-full"/> Project Overview
                </h4>
              ...
              {project.financialFramework.length > 0 && (
                <section>
                  <h4 className="flex items-center gap-2 text-lg font-bold text-slate-900 font-['Montserrat'] mb-6">
                    <span className="w-1 h-6 bg-green-500 rounded-full"/> Financial Framework
                  </h4>
                  <div className="grid sm:grid-cols-2 gap-4">
                    {project.financialFramework.map((f, i) => (
                      <div key={i} className="p-4 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col">
                        <span className="text-slate-500 text-xs font-bold uppercase tracking-widest mb-1">{f.source}</span>
                        <span className="text-xl font-black text-slate-900">{f.amount}</span>
                        {f.instrument && <span className="text-slate-400 text-xs mt-1">{f.instrument}</span>}
                      </div>
                    ))}
                  </div>
                </section>
              ...
              <section>
                <div className="flex items-center gap-2 text-slate-900 mb-4">
                  ...
                  <h4 className="text-lg font-bold font-['Montserrat']">Vision 2030 Alignment</h4>
                </div>
                <div className="flex flex-wrap gap-2">
                  {project.vision2030Pillars.map((p, i) => (
                    <span key={i} className="px-4 py-1.5 bg-red-50 text-red-700 rounded-full text-sm font-semibold border border-red-100">
                      {p}
                    </span>
                  ))}
                </div>
